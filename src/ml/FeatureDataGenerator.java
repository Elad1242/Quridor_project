...
public class FeatureDataGenerator {
    ...
    private static void playAndRecord(int oppType, Random rng,
        ...
        while (running && !state.isGameOver() && turns < MAX_TURNS) {
            if (state.getCurrentPlayerIndex() == 0) {
                running = recordBotTurn(state, bot, rng, allFeatures, allLabels, totalSamples);
            } else {
    ...
    // records BotBrain's chosen action + random alternatives, then applies the chosen action
    private static boolean recordBotTurn(GameState state, BotBrain bot, Random rng,
                                          List<double[]> allFeatures, List<Double> allLabels,
                                          AtomicInteger totalSamples) {
        try {
    ...
    private static double[] computeAltFeatures(GameState state, Object action) {
        if (action instanceof Position) return GameFeatures.extractForMove(state, (Position) action);
        if (action instanceof Wall)     return GameFeatures.extractForWall(state, (Wall) action);
        return null;
    }

    // picks 2 random actions different from BotBrain's choice
